...
foreach $chrstr(sort keys(%csi)) {
    ...
    foreach $id(keys(%{$csi{$chrstr}})) {
	    ...
	    $p = join(",", @list[0..$i]);
	    $q = join(",", @list[$i..(@list-1)]);
            $init{$list[$i]}{$p} = $term{$list[$i]}{$q} = TRUE;
        }
	...
	for($i=0;$i<@arr;$i++) {
		    ...
		    $p1 = join(",", sort {$a<=>$b} @site{@a});
                    ...
                    print join("\t", $chr, @site{@c[-1..0]}, $t, 2, $str, join(":", $p1, $p2)),"\n";
		}
	    }
	}
    }
